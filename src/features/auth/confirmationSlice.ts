...
export const resendCode = createAsyncThunk(
  ...
  async (_: void, { getState }) => {
    const { email, flow, pendingEmail } = (getState() as RootState)
      .confirmation;
    if (!email || !flow) throw new Error('NO_CONTEXT');

    if (flow === 'signup') await cognitoClient.resendConfirmationCode(email);
    else if (flow === 'forgot') await cognitoClient.forgotPassword(email);
...
export const confirmCode = createAsyncThunk<
  ...
  { state: RootState }
>('confirmation/confirm', async ({ code, newPassword }, { getState }) => {
  const { flow, email, pendingEmail } = getState().confirmation;
  if (!email || !flow) throw new Error('NO_CONTEXT');

  if (flow === 'signup') await cognitoClient.confirmSignUp(email, code);
  else if (flow === 'forgot' && newPassword)
...
const confirmationSlice = createSlice({
  ...
  reducers: {
    start(
      _,
      a: PayloadAction<{ flow: Flow; email: string; pendingEmail?: string }>,
    ) {
